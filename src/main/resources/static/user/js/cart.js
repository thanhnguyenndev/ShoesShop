$(document).ready(function () {
  ...
  $("#span1").attr('class').click(function () {
    count++;
    numbeBasket.text(count);
  });
  $("#span2").attr('class').click(function () {
    if (count <= 1) {
      return;
    } else {
      count--;
  ...
  $(".PLUS, .MINUS").click(function(){
       var itemVal = parseInt($(this).siblings(".display-quantity").val());
       if ($(this).hasClass('MINUS'))
            itemVal--;      
       else
            itemVal++;
       $(this).siblings(".display-quantity").val(itemVal);
    });
});
